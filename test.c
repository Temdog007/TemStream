    ...
    while (1) { /* we repeat if the bitstream is chained */
        ...
        while (i < 2) {
            while (i < 2) {
                ...
                if (result == 1) {
                    ogg_stream_pagein(&os, &og); /* we can ignore any errors
                                                    here as they'll also become
        ...
        if (vorbis_synthesis_init(&vd, &vi) == 0) { /* central decode state */
            ...
            while (!eos) {
                while (!eos) {
                    ...
                    ogg_stream_pagein(&os, &og); /* can safely ignore errors
                                                    at this point */
                    while (1) {
                        result = ogg_stream_packetout(&os, &op);
                        ...
                        if (result == 0)
                            break;        /* need more data */
                        if (result < 0) { /* missing or corrupt data at this
                                             page position */
                        ...
                        **pcm is a multichannel float vector.  In
                        stereo, for example, pcm[0] is left, and pcm[1]
                        is right.  samples is the size of each channel.
                        Convert the float values
                        ...
                        while ((samples = vorbis_synthesis_pcmout(&vd, &pcm)) >
                            ...
                            vorbis_synthesis_read(&vd,
                                                  bout); /* tell libvorbis how
                                                            many samples we
                                                            actually consumed */
                        }
                    }
            ...
            fprintf(stderr,
                    "Error: Corrupt header during playback initialization.\n");
        }
